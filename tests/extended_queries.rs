...
fn success_standard() {
    ...
    let query = query::parse_extended_query("abc=def&ab=ab%20cd%4A&arr[]=x&arr[]=y&arr[]=z&obj[a]=x&obj[b]=y&obj[c]=z");
...
fn success_ignore_incompatible() {
    ...
    let query = query::parse_extended_query("a[]=2&a[x]=3&a=1&b[]=1&b[x]=2&c[x]=1&c[]=2");
