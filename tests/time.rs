...
fn start_date() {
    ...
    let start_of_time = time::Time::now().with_stamp(0).format();
...
fn some_time_stamp() {
    ...
    let some_time = time::Time::now().with_stamp(6043440870).format();
    assert_eq!(some_time, "Sun, 05 Jul 2161 05:34:30 GMT");
}

#[test]
fn some_more_time_stamps() {
    let some_time = time::Time::now().with_stamp(333452334).format();
